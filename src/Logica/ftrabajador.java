...
public class ftrabajador {
    ...
    public DefaultTableModel mostrar(String buscar) {
        ...
        String[] titulos = {"idtrabajador", "Nombres", "Apellidos", "CI", "Telefono", "Direccion", "Email", "Sexo", "Login", "Password", "Acceso", "Estado", "Fecha_Ingreso"};
        String[] registros = new String[13];
        totalregistros = 0;
        modelo = new DefaultTableModel(null, titulos);
        sSQL = "select * from trabajador where CI like '%" + buscar + "%' order by idtrabajador";
    ...
    public boolean insertar(vtrabajador dts) {
        sSQL = "insert into trabajador (nombres,apellidos,CI,telefono,direccion,email,sexo,login,password,acceso,estado,fecha_ingreso)"
                + "values (?,?,?,?,?,?,?,?,?,?,?,?)";
        try {
    ...
    public boolean editar(vtrabajador dts) {
        sSQL = "update  trabajador set  nombres=?,apellidos=?,CI=?,telefono=?,direccion=?,email=?,sexo=?,login=?,password=?,acceso=?,estado=?,fecha_ingreso=?"
                + "where idtrabajador=?";
        try {
