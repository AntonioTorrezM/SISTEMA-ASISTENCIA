...
public class falumno {
   ...
   private Connection cn=mysql.conectar();
    ...
    public DefaultTableModel mostrar(String buscar){
        ...
        String [] titulos={"idAlumno","Nombres","Apellidos","CI","Direccion","Sexo","Fecha Nacimiento","Fecha Registro","Grado","Seccion"};
        String [] registros=new String [10];
        totalregistros=0;
        modelo=new DefaultTableModel(null,titulos);
        sSQL="select * from alumno where CI like '%"+ buscar +"%' order by idalumno";
    ...
    public DefaultTableModel mostrarvista_salidaalumno(String buscar){
     ...
     public  boolean insertar(valumno dts){
        sSQL= "insert into alumno (nombres,apellidos,CI,direccion,sexo,fecha_nacimiento,fecha_registro,grado,seccion)" +
        "values (?,?,?,?,?,?,?,?,?)";
        try {
      ...
      public  boolean editar(valumno dts){
        sSQL= "update  alumno set  nombres=?,apellidos=?,CI=?,direccion=?,sexo=?,fecha_nacimiento=?,fecha_registro=?,grado=?,seccion=?" +
        "where idalumno=?";
        try {           
